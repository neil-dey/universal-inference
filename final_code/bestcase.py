...
def offline_gue(data, true_value, alpha):
    bootstrap_iters = 100
    omegas = np.linspace(0.001, 10, 10000)
...
def online_gue(data, true_value, alpha):
    boot_iters = 100
    coverages = []
    omegas = np.linspace(0, 1, num = 1000)
...
def mc_iteration(nom_coverage, iteration_num):
    ...
    n1 = 0
    while True:
        # If we have enough data to run offline GUe
        if n0 >= 4 and n1 >= 4:
        ...
        # If the confidence interval still includes the origin, or we don't have enough data yet, collect more data and try again
        if (n0 < 4 or n1 < 4) or (CI[0] <= 0 and 0 <= CI[1]):
    ...
    # Universal interval
    #print("    Sample Size:", str(n0+n1))#, os.getpid())
    online_coverage = online_gue(np.array(list(zip(data, datasequence))), c, 1 - nom_coverage)
